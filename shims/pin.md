...
allowed-tools: Bash(pin:*), AskUserQuestion
---

Pin the current Claude Code session. Session ID: `${CLAUDE_SESSION_ID}`.
Current pin status (from the store): !`pin _status "${CLAUDE_SESSION_ID}"`
...
4. Run exactly:
   ...
   ```
   pin add "${CLAUDE_SESSION_ID}" <alias> --title "<title>"
   ```
...
<!-- claude-pins shim: generated from commands/pin.md by tests/shims_check.py; installed by /pins:pins-install -->
