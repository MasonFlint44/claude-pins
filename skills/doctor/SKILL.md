...
3. **Explain each non-✓ line and its fix:**
   ...
   | `· keep: … the pins plugin is not enabled` | Claude's `settings.json` does not enable the plugin (`pin` runs from a checkout or a stale symlink), so its session-start hook never runs and keep pins are touched only when `pin` itself runs | `/plugin install pins@claude-toolbox` (or enable it in `/plugin`), then restart Claude; `/hooks` then lists the pins SessionStart hook |
